...
Page({
  data: {
    ...
    voiceLists: ["12421421", "314y21ihjksd", "撒旦飞洒发的我的空拉斯克斯"],
    voiceLen: 0,
    animationNote: {}
  },
    ...
    wx.request({
      ...
      success:function(res){
        ...
        that.setData({
          ...
          arid:video.arid,
          row: video.row,
          count: video.row.count
        })
        that.Zan();
        that.ShouCang();
        console.log('video.row.count', video.row.count)
      },
    ...
    wx.request({
      ...
      success:function(res){
        that.setData({
          ...
          voice: res.data[0].post_desc,
          arid: res.data[0].arid
        })
      }
    })
  },
  //广播跳转
  voiceTap(e) {
    var arid = e.currentTarget.dataset.id
    console.log(arid)
    // wx.navigateTo({
    wx.redirectTo({
      url: '/pages/video/video?id=' + arid,
    })
  },
  //點讚
  Zan: function () {
  ...
  onShow: function () {
    ...
    var noteLen = this.data.voiceLists.length - 1;
    console.log(noteLen)
    ...
    setInterval(function () {
      if (next == true) {
        ++this.data.voiceLen;
        if (this.data.voiceLen >= noteLen) {
          // this.data.voiceLen = 0
          next = false
        }
        this.animation.top(-18 * this.data.voiceLen).step()
      } else {
        --this.data.voiceLen;
        if (this.data.voiceLen <= 0) {
          // this.data.voiceLen = noteLen
          next = true
        }
        this.animation.top(-18 * this.data.voiceLen).step()
      }
      this.setData({
        animationNote: animation.export()
      })
    }.bind(this), 5000)
  },
})
