...
Page({
  ...
  Surrounding(e){
    ...
    var that = this;
    var latitude = e.currentTarget.dataset.latitude; //纬度
    var longitude = e.currentTarget.dataset.longitude;//经度
    wx.request({
      // 必需
      url: app.d.ceshiUrl + '/Api/Index/periphery',
      method: 'POST',
  ...
  toBuy(e){
    ...
    wx.navigateTo({
      url: '/pages/indexBuy/indexBuy?pid='+pid,
    })
  },
  // 评论
  toComment(e){
  ...
  getDirect(start, end) {
    ...
    if (Math.abs(X) > Math.abs(Y) && X > 0) {
      ...
      var userid = app.d.userId;
      wx.navigateTo({
        url: '/pages/userinfo/userinfo?id=' + user_id,
      })
      console.log("left");
    }
  ...
  onShow: function () {
    var that = this;
    that.videoCtx = wx.createVideoContext('myVideo');
